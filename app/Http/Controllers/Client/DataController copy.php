...
class DataController extends Controller
{
    ...
    public function createOrder(Request $request): JsonResponse
    {
        try {
            ...
            // Auto-set reactType from service type_radio if not provided
            if (!$request->filled('reactType') && $service->type_radio) {
                $request->merge(['reactType' => $service->type_radio]);
            }
            ...
            $orderData = [
                ...
                'reaction' => $request->reactType ?? $service->reaction ?? null,
            ];

            // Add dripfeed settings
            ...
            // Add schedule time
            if ($request->filled('delay') && $request->delay > 0) {
            ...
            $additionalFields = ['expiry', 'username', 'keywords', 'comments', 'hashtag', 'hashtags', 'usernames', 'mediaUrl', 'posts', 'old_posts', 'min', 'max', 'country', 'device', 'type_of_traffic', 'google_keyword', 'referring_url', 'reactType'];
            ...
            DB::transaction(function () use ($orderData, $user, $total, $service, $request, &$order, &$apiResponse, &$apiError, &$isErrorKept) {
                ...
                if ($provider && $provider->status) {
                    ...
                    // Add reaction from service if exists
                    if ($service->reaction) {
                    ...
                    switch ($serviceType) {
                        case 'Custom Comments':
                            ...
                            // Add reactType if provided (override service reaction)
                            if ($request->filled('reactType')) {
                                $apiData['reaction'] = $request->reactType;
                            }
            ...
            // Format values
            $formatValue = fn($v) => rtrim(rtrim(number_format($v, 7), '0'), '.') ?: '0';
            ...
            $orderResponse = [
                ...
                'charge' => '$' . $formatValue($total),
                'original_charge' => '$' . $formatValue($rate),
                'original_balance' => '$' . $formatValue($user->balance + $total),
                'balance' => '$' . $formatValue($user->balance),
                'converted' => false,
