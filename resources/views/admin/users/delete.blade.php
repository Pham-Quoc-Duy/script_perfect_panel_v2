<script>
    // Delete user function - Based on exact t.html AlertifyJS confirm implementation
    window.deleteUser = function(userId, userName = '') {
        ...
        alertify.confirm('Xác nhận xóa thành viên', `Bạn có chắc chắn muốn xóa thành viên "${userName}" không?`,
            function() {
                // User confirmed - exactly like t.html success callback
                const form = document.getElementById('delete-form-' + window.currentUserId);
                if (form) {
                    alertify.success('Đang xóa thành viên...');
                    form.submit();
                } else {
    ...
    document.addEventListener('DOMContentLoaded', function() {
        document.addEventListener('click', function(e) {
            ...
            if (btn) {
                ...
                const userName = btn.dataset.userName || btn.dataset.userEmail || 'người dùng này';
